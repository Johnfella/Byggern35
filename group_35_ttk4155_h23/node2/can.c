...
void can_data_direct(can_data *data, CanMsg *new_message) {
    ...
    if (new_message->id == 20) {
        ...
        data->servo = mapValue(data->joystick_horizontal,255,0,900,2100);

    } else if (new_message->id == 10) {
        data->game_active = new_message->byte[0];
        data->game_init = new_message->byte[1];
        data->game_ready = new_message->byte[2];
        data->game_score = new_message->byte[3];
    }
}
